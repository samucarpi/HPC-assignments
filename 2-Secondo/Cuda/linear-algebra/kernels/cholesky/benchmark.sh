...
OPTIMIZATION_FLAGS=("SEQUENTIAL" "OPTIMIZED_v1" "OPTIMIZED_v2") # ottimizzazioni disponibili
DATASETS=("MINI" "SMALL" "STANDARD" "LARGE" "EXTRALARGE") # dataset disponibili
EXECUTABLE="./cholesky.exe" # eseguibile CUDA
...
for opt_flag in "${OPTIMIZATION_FLAGS[@]}"; do
  ...
  # compilazione
  CFLAGS_BUILD="-DPOLYBENCH_TIME -D${opt_flag} ${DATASET_DEFINE_OPTION}"
  if ! make FLAGS="$CFLAGS_BUILD" clean all > build.log 2>&1; then
